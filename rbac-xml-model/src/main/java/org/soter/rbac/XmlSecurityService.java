...
public class XmlSecurityService implements SecurityService {
    ...
    public XmlSecurityService(RbacType rbac) {
        this.rbac = rbac;
    }

    public boolean check(UserInfo user, Permission permission, String scope) {
        List<RoleType> roles = rbac.getUserRoles(user.getUserName(), user.getPassword());
